...
if DEV_MODE:
    _component_func = components.declare_component(
        "dir_control_component",
        url="http://localhost:3001", #npm run start with deploy component here
    )

else:
    _component_func = components.declare_component("dir_control_component",path=build_dir)
...
def dir_control_component(max_value,min_value,default_value,increment,interval_ms, key=None):
    ...
    component_value = _component_func(max_value=max_value,min_value=min_value,default_value=default_value,increment=increment,interval_ms=interval_ms,key=key, default=90)
